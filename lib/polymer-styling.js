...
Polymer.CssParse = (function() {
  ...
  return {
    ...
    parse: function(text, buildType) {
      text = this._clean(text);
      return this._parseCss(this._lex(text), text, buildType);
    },
    ...
    _parseCss: function(node, text, buildType) {
      var t = text.substring(node.start, node.end-1);
      node.parsedCssText = node.cssText = t.trim();
      node._buildType = buildType;
      if (node.parent) {
      ...
      if (r$) {
        for (var i=0, l=r$.length, r; (i<l) && (r=r$[i]); i++) {
          this._parseCss(r, text, buildType);
        }
      }
      return node;
    },
    ...
    return {
      ...
      rulesForStyle: function(style) {
        if (!style.__cssRules && style.textContent) {
          var buildType = this.getCssBuildType(style);
          style.__cssRules = this.parser.parse(style.textContent, buildType);
        }
    ...
    */
    var api = {
      ...
      elementStyles: function(element, callback) {
        ...
        var cssText = '';
        var cssBuildType = styleUtil.cssBuildTypeForModule(element.is);
        for (var i=0, l=styles.length, s; (i<l) && (s=styles[i]); i++) {
          var rules = styleUtil.rulesForStyle(s, cssBuildType);
...
Polymer.ApplyShim = (function(){
  ...
  function produceCssProperties(matchText, propertyName, valueProperty, valueMixin) {
    ...
    for (p in mixinValues) {
      v = mixinValues[p];
      // if property not defined by current mixin, set initial
      if (v === undefined) {
        v = 'initial';
      }
      out.push(propertyName + '-' + p + ': ' + v);
    }
  ...
  // @apply --foo; -> for all props (${propname}: var(--foo-${propname}, ${fallback[propname]}}))
  // Example:
  // border: var(--foo-border); padding: var(--foo-padding, 2px)
