...
Polymer.ApplyShim = (function(){
  ...
  // diffProperties([a, b], [b]) -> [a]
  function diffProperties(oldSet, newSet) {
    var diff = [];
  ...
  // produce variables in place of a mixin definition
  function defineMixinProperties(mixinName, propNames, propMap, oldProps) {
    var out = [];
    var i, p, v;
    ...
    var resetProps = diffProperties(oldProps, propNames);
  ...
  // textToProps('border: 2px solid black') -> ('border', '2px solid black')
  ...
  function mixinAssignment(matchText, propertyName, valueProperty, valueMixin) {
    ...
    var mixinAsProperties = mixinApplication(valueMixin);
    var prefix = matchText.slice(0, matchText.indexOf('--'));
    var mixinProps = textToProps(mixinAsProperties);
    var propNames = Object.keys(mixinProps);
    var oldProps = mapGet(propertyName);
    mapSet(propertyName, propNames);
    return prefix + defineMixinProperties(propertyName, propNames, mixinProps, oldProps);
  }
  ...
  // @apply --foo; -> forAll ${propname} in --foo ~> --foo-${propname}: ${value}
  function applyMixinProperties(mixinName, fallbacks) {
    mixinName = mixinName.replace(APPLY_NAME_CLEAN, '');
    var out = '';
    var mixinProperties = mapGet(mixinName);
    if (mixinProperties) {
      var vars = [];
      for (var i = 0, p, f, parts; i < mixinProperties.length; i++) {
        p = mixinProperties[i];
        f = fallbacks && fallbacks[p];
        parts = [p, ': var(', mixinName, '-', p];
        // if a property is defined in this rule, use it as a fallback to this variable
        if (f) {
          parts.push(',', f);
        }
        parts.push(')');
  ...
  function mixinApplication(text) {
    var m;
    ...
    while((m = MIXIN_MATCH.exec(text))) {
      ...
      var defaults = textToProps(textBeforeApply);
      var replacement = applyMixinProperties(mixinName, defaults);
  ...
  var ApplyShim = {
    ...
    transformCssText: function(cssText) {
      ...
      // produce variables
      cssText = cssText.replace(VAR_ASSIGN, mixinAssignment);
      // consume mixins
      return mixinApplication(cssText);
    }
  };
